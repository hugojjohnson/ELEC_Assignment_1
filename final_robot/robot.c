...
void robotAutoMotorMove(struct Robot *robot, int front_centre_sensor, int left_sensor, int right_sensor) {
    ...
    // h: Happy. It's just moving forwards. It will try and put itself in the optimal position.
    ...
    switch (robot->context) {
        ...
        case 'h':
            switch(robot->count) {
                case 0:
                    if (right_sensor <= 1) {             // too far
                        robot->direction = RIGHT;
                        // robot->count = 1;
                        break;
                    } else if (right_sensor == 4) {     // too too close
                        robot->direction = LEFT;
                        robot->count++;
                        // do something
                        break;
                    } else if (right_sensor >= 3) {      // too close
                        robot->direction = LEFT;
                        break;
                    } else {                            // otherwise
                        robot->direction = UP;
